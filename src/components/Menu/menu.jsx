...
>
  ...
  <div
    ...
    style={{
      backgroundImage:
        "url('https://images.pexels.com/photos/1199960/pexels-photo-1199960.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2')",
    }}
  >
    {/* Heading */}
    <h2 className="text-4xl font-bold text-white mb-10 mt-8 italic underline tracking-wide">
    ...
    <div className="relative w-full max-w-6xl flex items-center justify-center h-[400px] px-6">
      ...
      <button
        onClick={prev}
        className="absolute left-0 z-10 bg-white text-black p-3 rounded-full shadow-lg hover:bg-gray-100 transition"
      >
        ‹
      </button>
      ...
      <button
        onClick={next}
        className="absolute right-0 z-10 bg-white text-black p-3 rounded-full shadow-lg hover:bg-gray-100 transition"
      >
        ›
      </button>
    </div>
    ...
    <div key={index} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-2xl transition duration-3 w-64 p-5">
       ...
       {/* Container for image and rotating border */}
        <div className="relative w-36 h-36 mx-auto mb-4 flex foods-center justify-center">
          ...
          <div className="absolute w-32 h-32 mt-2 rounded-full overflow-hidden z-10"><img
  onClick={() => navigate("/product-detail", { state: food })}
  src={food.image}
  alt={food.name}
  className="cursor-pointer w-full h-48 object-cover transform transition-transform duration-300 hover:scale-105 hover:brightness-110"
